...
- **Não consigo publicar no GitHub.** `git push` (sem credenciais) e as
  ferramentas MCP (`push_files`, `create_or_update_file`) retornam
  `403 Resource not accessible by integration`: a integração do GitHub App
  desta sessão não tem permissão de escrita no repo. Todo o trabalho acima
  está commitado localmente na branch `claude/bootstrap-veiculo-scraper-3lzi2y`
  (2 commits: Passo 0 e o scaffold; a Fase 1 completa ainda por commitar).
